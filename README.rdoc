...
* One must both include and extend NamedValues in order to get lowercase instances.
* If you run into this error: "super from singleton method that is defined to multiple classes is not supported; this will be fixed in 1.9.3 or later" try declaring instances directly after the NamedValueClass call for each new type.  I'm still trying to track down a solution to this (unless 1.9.3 beats me).

...
== Rg operators the blocks are always yieled: the LHS (left hand side), a proc with the original, default implementation of the operator you are redefining, and the RHS.  The passed proc's receiver is the LHS so in the following code "minus.call(rhs)" performs "lhs - rhs".  
